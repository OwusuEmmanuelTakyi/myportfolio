        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              exit={{ x: "100%" }}
              transition={{ duration: 0.3, ease: "easeInOut" }}
              className="fixed inset-y-0 right-0 z-40 flex w-3/4 max-w-xs flex-col gap-6 border-l border-brand-border bg-brand-surface p-8 pt-24 shadow-2xl md:hidden"
            >
              {links.map((link) => (
                <Link
                  key={link.href}
